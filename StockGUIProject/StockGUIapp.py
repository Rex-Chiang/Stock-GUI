...
class StockGUI():
    def __init__(self):
        ...
        self.stocks = wx.StaticText(
            parent=self.panel,
            label=f"YOUR STOCKS : {str(self.Crawler.stocks)}",
            pos=(10, 45)
        )
    ...
    def Input(self, event):
        ...
        self.stocks.SetLabel(label=f"YOUR STOCKS : {str(self.Crawler.stocks)}")
    ...
    def Refresh(self, event):
        try:
            ...
            for i in range(0, len(self.Crawler.stocks)):
                ...
                image = wx.Image(f"stock_screenshot/{self.Crawler.stocks[i]}.png", wx.BITMAP_TYPE_PNG)
                ...
                if self.Crawler.stocks[i] not in self.bmps_dict:
                    self.bmps_dict[self.Crawler.stocks[i]] = wx.StaticBitmap(
                        parent=self.frame,
                        bitmap=bitmap,
                        pos=(5, interval),
                        size=(10, 10)
